...
static int run_producer (const char *topic, int msgcnt,
			 ...
			 const char* device_id, const char* device_name, const char* device_type, const char* url, const char* myTime, const char* myMessage) {
        rd_kafka_t *rk;
        char errstr[512];
        int i;
		...
		char key[1024];
		int t = (int)time(NULL);
		sprintf(key, "%s-%lu", device_id, t);
                snprintf(json, sizeof(json),
                         "{ \"event_id\": \"%s\", \"device_id\": \"%s\", \"device_name\": \"%s\", \"device_type\": \"%s\", \"url\": \"%s\", \"time\": \"%s\", \"message\": \"%s\" }", 
					  key, device_id, device_name, device_type, url, myTime, myMessage);
...
int main (int argc, char **argv) {
        ...
        if (argc != 9) {
                fprintf(stderr, "Usage: %s <topic> <config-file> <device_id> <device_name> <device_type> <url> <time> <message>\n", argv[0]);
                exit(1);
        }
	...
	const char* device_type = argv[5];
	const char* url = argv[6];
	const char* time = argv[7];
	const char* message = argv[8];

        if (run_producer(topic, 1, conf, device_id, device_name, device_type, url, time, message) == -1)
                return 1;
